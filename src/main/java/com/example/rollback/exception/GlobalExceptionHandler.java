 ...
 * 
 * <p>이 클래스는 모든 REST 컨트롤러에서 발생하는 예외를 중앙에서 처리하여
 * 다음과 같은 이점을 제공합니다:</p>
 * <ul>
...
public class GlobalExceptionHandler {
     ...
     * 
     * <p>이 메서드는 {@link org.springframework.validation.annotation.Valid} 어노테이션이
     * 적용된 DTO 객체의 필드 검증이 실패했을 때 발생하는 예외를 처리합니다.</p>
     * 
     * <p>처리 기능:</p>
     * <ul>
     ...
     * 
     * <p>이 메서드는 애플리케이션에서 발생하는 모든 일반 예외를 최후의 수단으로 처리합니다.
     * 시스템의 안정성을 보장하고 내부 정보 노출을 방지하기 위해
     * 일반화된 안전한 메시지를 클라이언트에 전달합니다.</p>
     * 
     * <p>처리 기능:</p>
     * <ul>
     ...
     * 
     * <p>이 메서드는 애플리케이션의 비즈니스 규칙 위배나
     * 데이터 상태 관련 문제로 발생하는 예외를 처리합니다.</p>
     * 
     * <p>처리 대상 예외:</p>
     * <ul>
