 ...
 * 
 * <p>트랜잭션 실패 시 고객에게 알림을 전송하고, 모든 알림 기록을 데이터베이스에 저장합니다.
 * 새로운 트랜잭션(REQUIRES_NEW)에서 실행되어 원본 트랜잭션 롤백에 영향을 받지 않습니다.</p>
 * 
 * <h3>주요 기능:</h3>
 * <ul>
 ...
 * </ul>
 * 
 * <p><strong>트랜잭션 정책:</strong><br>
 * REQUIRES_NEW propagation을 사용하여 호출하는 트랜잭션과 별개의 새로운 트랜잭션에서 실행됩니다.
 * 이를 통해 원본 트랜잭션이 롤백되더라도 알림 로그는 정상적으로 저장됩니다.</p>
 * 
...
public class NotificationService {
     ...
     * 
     * <p>기존 주문 시스템과의 호환성을 위해 제공되는 메서드입니다.
     * 새로운 트랜잭션에서 실행되어 주문 트랜잭션 롤백에 영향을 받지 않습니다.</p>
     * 
     ...
     * 
     * <p>은행 거래 실패 시 고객에게 SMS와 이메일로 알림을 전송하고, 
     * 알림 내역을 데이터베이스에 기록합니다. 
     * 새로운 트랜잭션에서 실행되어 거래 트랜잭션 롤백에 영향을 받지 않습니다.</p>
     * 
